...
byte c;
  ...
  Serial.println("IC-Tester V0.5");
  lcd.setCursor(0,0); 
  lcd.print("IC-Tester V0.5");
  ...
  while(PrintPart[startP]!='$'){
    startP++;
    PrintPart[startP]=(char)(i2c_eeprom_read_byte(0x50, startP));
  }  
  PrintPart[startP-2]=0x00;                                           // -2 for \r\n
...
char c;
  ...
  while(run==HIGH){
    ...
    if(run==HIGH){                                                     // if it is still high
      ...
      c=(char)(i2c_eeprom_read_byte(0x50, j));                        // first digit of amount of pins
